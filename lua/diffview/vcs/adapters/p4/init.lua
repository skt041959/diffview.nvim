...
local Commit = require("diffview.vcs.adapters.p4.commit").P4Commit
...
P4Adapter.file_history_worker = async.void(function(self, out_stream, opt)
    local single_file = #opt.log_opt.single_file.path_args == 1 -- Basic check
        ...
        for _, file_info in ipairs(files_in_cl) do
            ...
            if #state.path_args > 0 then
                 ...
                 for _, arg_path in ipairs(state.path_args) do
                     -- Assuming file_info.path is a depot path like //depot/main/file.c
                     -- And arg_path might be similar or a local path
                     -- This comparison logic needs refinement based on how paths are handled.
...
function P4Adapter:is_binary(path, rev)
    ...
    elseif rev and rev.type == RevType.LOCAL then
        -- Check local file if possible, fallback to head? `p4 fstat` on local path works.
        path_spec = path
    else
        path_spec = path .. "#head" -- Default check against head
    end

    local fstat_job = Job({
        command = self:bin(),
        args = { "fstat", "-T", "headType", path_spec }, -- headType gives type at #head
        cwd = self.ctx.toplevel,
        log_opt = { label = "P4Adapter:is_binary(fstat)" },
    })

    local ok, _ = await(fstat_job)
    if not ok or fstat_job.code ~= 0 then
        return true -- Assume binary or non-existent on error
    end

    for _, line in ipairs(fstat_job.stdout) do
        local file_type = line:match("headType (%S+)")
        if file_type then
            -- Check for common binary types or types needing special handling
            if file_type:find("binary") or file_type:find("apple") or file_type:find("resource") or file_type:find("unicode") or file_type:find("utf16") then
                return true
            else
                return false -- Assume text otherwise
            end
        end
    end
